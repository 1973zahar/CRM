  ...
  if (photos.length) {
    return `
      <div class="catalog-photo">
        <img src="${escapeHtml(photos[0].dataUrl)}" alt="${escapeHtml(photos[0].name || product.model)}">
      </div>
      ${photos.length > 1 ? `
        <div class="catalog-photo-strip">
          ${photos.slice(1, 6).map((photo) => `<img src="${escapeHtml(photo.dataUrl)}" alt="${escapeHtml(photo.name || product.model)}">`).join("")}
        </div>
      ` : ""}
    `;
  }
...
function renderB2B() {
  ...
  const filteredResponsibleRows = responsibleRows.filter((row) => dateInPeriod(row.date, docsPeriod));
  const shipmentRequestRows = b2bShipmentRequestRows();
  const filteredShipmentRequestRows = shipmentRequestRows.filter((row) => dateInPeriod(row.date, docsPeriod));
  ...
  return `
    ...
    <section class="panel section-band">
      ...
      <div class="table-wrap">
        <table>
          <thead><tr><th>Дата</th><th>Клієнт</th><th>Товар</th><th>К-сть</th><th>Бажана дата</th><th>Статус</th><th>Перевірка</th><th>Дії</th></tr></thead>
          <tbody>
            ${filteredShipmentRequestRows.map((row) => `
              <tr>
                <td><strong>${row.id}</strong><br><span class="small muted">${row.date}</span></td>
                ...
                <td class="row-actions">
                  <button class="secondary" data-open-b2b-shipment-request="${row.id}" ${["request_approved", "request_rejected", "request_cancelled"].includes(row.status) ? "disabled" : ""}>Підготувати</button>
                  <button class="ghost danger" data-reject-b2b-shipment-request="${row.id}" ${["request_approved", "request_rejected", "request_cancelled"].includes(row.status) ? "disabled" : ""}>Відхилити</button>
                </td>
              </tr>
            `).join("") || '<tr><td colspan="8" class="muted">Заявок B2B на відвантаження за вибраний період немає.</td></tr>'}
          </tbody>
        </table>
      </div>
    </section>
    ...
    <section class="panel section-band" data-print-area="b2bDocs" data-print-title="B2B відповідальне зберігання">
      ...
      ${renderPeriodPrintControls("b2bDocs", "B2B відповідальне зберігання", docsPeriod, filteredResponsibleRows.length)}
      <div class="table-wrap">
        <table>
          <thead><tr><th>Документ</th><th>Дата</th><th>Клієнт / склад</th><th>Товар</th><th>Передано</th><th>Продано</th><th>Залишок</th><th>Серії</th><th>Оплата після продажу</th><th>Статус</th><th>Дії</th></tr></thead>
          <tbody>
            ${filteredResponsibleRows.map((row) => `
              <tr>
                ...
                <td class="row-actions no-print"><button class="ghost" data-edit-responsible-doc="${row.id}" ${canEditPostedDocument("responsibleShipment") ? "" : "disabled"}>Змінити</button></td>
              </tr>
            `).join("") || '<tr><td colspan="11" class="muted">Документів відповідального зберігання за вибраний період немає.</td></tr>'}
          </tbody>
        </table>
      </div>
    </section>
...
function renderMarketplaces() {
  ...
  const filteredPublications = state.marketplacePublications.filter((publication) => dateInPeriod(String(publication.lastSync || today).slice(0, 10), docsPeriod));
  ...
  return `
    ...
    <section class="panel section-band" data-print-area="marketplaceDocs" data-print-title="Маркетплейси · публікації та замовлення">
        ...
        <table>
          ...
          <tbody>
            ${filteredPublications.map((publication) => {
              ...
              `;
            }).join("") || '<tr><td colspan="9" class="muted">Немає публікацій за вибраний період.</td></tr>'}
          </tbody>
        </table>
      </div>
...
document.addEventListener("click", (event) => {
  ...
  if (target.dataset.resetMarketplaceOrderFilter !== undefined) {
    state.marketplaceOrderFilters = { from: "", to: "", status: "", marketplace: "" };
    render();
    return;
  }
